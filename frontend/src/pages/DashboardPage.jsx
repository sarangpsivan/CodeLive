...
const DashboardPage = () => {
    ...
    return (
        // This is the fix: Added 'text-white' to the main container
        // and 'h-full' to make it fill the space from the new layout
        <main className="flex flex-col lg:flex-row gap-8 p-8 font-sans text-white h-full">
            {/* Sidebar Area */}
            <aside className="w-full lg:w-80 bg-[var(--dark-card)] rounded-xl p-6 border border-gray-800 flex flex-col flex-shrink-0">
                <h2 className="text-lg font-semibold mb-6 px-3">Your Projects</h2>
                ...
                <button className="w-full mt-6 py-3 flex items-center justify-center bg-[var(--primary-purple)] font-bold rounded-lg hover:brightness-110 transition-colors">
            ...
            <section className="flex-1 overflow-y-auto">
                <div className="mb-10">
                    <h1 className="text-4xl font-bold">Welcome back, <span className="text-[var(--accent-lavender)]">{user?.first_name || user?.username}</span>!</h1>
